...
public class BotWars extends BaseGameActivity implements
	...
	private static final short MASKBITS_BULLET = CATEGORYBIT_WALL
			+ CATEGORYBIT_ENEMY;
	...
	private static String mapBG = "tmx/background.png";
	...
	private static float mLinearVelocityY = 8.0f;
	...
	/**********************************************************************************
	 * 
	 * Override onLoadEngine method of AndEngine Create the engine and camera,
	 * check multitouch support and set engine options here
	 * 
	 **********************************************************************************/
	@Override
		...
		try {
			if (MultiTouch.isSupported(this)) {
				...
				if (MultiTouch.isSupportedDistinct(this)) {
					...
					Toast.makeText(
							this,
							"(MultiTouch detected, but your device might have problems to distinguish between separate fingers.)",
	...
	/**********************************************************************************
	 * 
	 * Override onLoadResources method of AndEngine Load all textures, maps,
	 * sounds and fonts here
	 * 
	 **********************************************************************************/
	@Override
	...
	/**********************************************************************************
	 * 
	 * Override onLoadScene method of AndEngine Initialize everything here
	 * 
	 * Create a Scene and PhysicsWorld Attach all sprites, buttons and map to
	 * scene Start music Create and register game loop UpdateHandler Create and
	 * register ContactListener Create and register scrollDetector and
	 * pinchZoomDetector Put all sprites into mEntityList Set Camera bounds
	 * 
	 * 
	 **********************************************************************************/
	@Override
	public Scene onLoadScene() {
		...
		player_self_body = mPhysicsWorld.getPhysicsConnectorManager()
				.findBodyByShape(player_self_sprite);
		...
		mCamera.setBounds(0,
				mTMXTiledMap.getTileColumns() * mTMXTiledMap.getTileWidth(), 0,
				mTMXTiledMap.getTileRows() * mTMXTiledMap.getTileHeight());
	...
	private void createCollisionListener() {
		this.mPhysicsWorld.setContactListener(new ContactListener() {
			...
			public void beginContact(Contact contact) {
				...
				// get UserData of the two bodies between which collision is
				// detected
				...
				if ((fix1_name.contains("player_self") && fix2_name
						.contains("wall"))
						|| (fix2_name.contains("player_self") && fix1_name
								.contains("wall"))) {
					isLanded = true;
				}

				// collision between enemy and wall
				if ((fix1_name.contains("enemy") && fix2_name.contains("wall"))
						|| (fix2_name.contains("enemy") && fix1_name
								.contains("wall"))) {
					enemyLanded = true;
...
////
				...
				if ((fix1_name.contains("bullet") && fix2_name
						.equalsIgnoreCase("wall"))
						|| (fix2_name.contains("bullet") && fix1_name
								.equalsIgnoreCase("wall"))) {
				...
				if ((fix1_name.contains("bullet_self") && fix2_name
						.contains("enemy"))

						|| (fix2_name.contains("bullet_self") && fix1_name
								.contains("enemy"))) {
					enemyShot = true;
				}
	 ...
	 * 
	 **********************************************************************************/
	private void makeRectanglesFromObjects(TMXObjectGroup _group,
			String _userData) {
	...
	public void initControls() {
		...
		Sprite shoot = new Sprite(CAMERA_WIDTH - 120, CAMERA_HEIGHT - 110,
				mShootTextureRegion) {
			@Override
		...
		Sprite move_up = new Sprite(60, CAMERA_HEIGHT - 210,
				mShootTextureRegion) {
			@Override
			public boolean onAreaTouched(TouchEvent pEvent, float pX, float pY) {
				...
				if(pEvent.isActionDown()) {
					 ...
					 */
					 if (!player_self_sprite.isAnimationRunning()) {
						movePlayerUp(player_self_sprite, player_self_body);
					 }
					isButtonAreaTouched = true;
					isPlayerMoving=true;
				}
				...
				if (pEvent.isActionUp()) {
				//if (isPlayerMoving) {
						stopPlayerY(player_self_sprite, player_self_body);
		...
		move_up.setScale(0.50f);
		mHUD.registerTouchArea(move_up);
		mHUD.attachChild(move_up);
		
		Sprite move_down = new Sprite(60, CAMERA_HEIGHT - 110,
				mShootTextureRegion) {
			@Override
			public boolean onAreaTouched(TouchEvent pEvent, float pX, float pY) {
				...
				if (pEvent.isActionDown()) {
					 ...
					 */
					 if (!player_self_sprite.isAnimationRunning()) {
						movePlayerDown(player_self_sprite, player_self_body);
					 }
					isButtonAreaTouched = true;
				}
		...
		move_down.setScale(0.50f);
		mHUD.registerTouchArea(move_down);
		mHUD.attachChild(move_down);
		
		Sprite move_left = new Sprite(10, CAMERA_HEIGHT - 160,
				mShootTextureRegion) {
			@Override
			public boolean onAreaTouched(TouchEvent pEvent, float pX, float pY) {
				...
				if (pEvent.isActionUp()) {
				//	if (isPlayerMoving) {
						stopPlayerX(player_self_sprite, player_self_body);
						isPlayerMoving = false;
						isButtonAreaTouched = false;
					//}
				}
		...
		move_left.setScale(0.50f);
		mHUD.registerTouchArea(move_left);
		mHUD.attachChild(move_left);
		
		Sprite move_right = new Sprite(110, CAMERA_HEIGHT - 160,
				mShootTextureRegion) {
			@Override
			public boolean onAreaTouched(TouchEvent pEvent, float pX, float pY) {
		...
		move_right.setScale(0.50f);
		mHUD.registerTouchArea(move_right);
		mHUD.attachChild(move_right);
	...
	private void loadCharacters() {
		...
		this.mPlayer_MPTextureRegion = BitmapTextureAtlasTextureRegionFactory
				.createTiledFromAsset(this.mBitmapTextureAtlas, this,
						"andrun256_red.png", 0, 64, 8, 1);

		this.mEnemyTextureRegion = BitmapTextureAtlasTextureRegionFactory
				.createTiledFromAsset(this.mBitmapTextureAtlas, this,
						"bananaHQ_tiled.png", 0, 128, 4, 2);

		this.mBulletTextureRegion = BitmapTextureAtlasTextureRegionFactory
				.createTiledFromAsset(this.mBitmapTextureAtlas, this,
						"bullet.png", 0, 256, 1, 1);
	...
	private void loadControls() {
		...
		this.mHUDTextureAtlas = new BitmapTextureAtlas(512, 1024,
				TextureOptions.BILINEAR_PREMULTIPLYALPHA);
	...
	private void loadSounds() {
		try {
			...
			mMusic = MusicFactory.createMusicFromAsset(
					this.mEngine.getMusicManager(), this, "mi.ogg");
			mMusic.setLooping(true);

			mWalkSound = SoundFactory.createSoundFromAsset(
					this.mEngine.getSoundManager(), this, "fire_alarm4.wav");

			mShootSound = SoundFactory.createSoundFromAsset(
	...
	/**********************************************************************************
	 * 										->
	 * Spawn bullet in front of given player and set UserData string (name) and
	 * play shoot sound
	 * 
	 **********************************************************************************/
	...
	public void spawnBullet(AnimatedSprite _playerSprite, int _playerDir,
		...
		AnimatedSprite mBulletSprite = new AnimatedSprite(_playerSprite.getX()
				+ _playerDir, _playerSprite.getY() + _playerSprite.getHeight()
				/ 4, mBulletTextureRegion);
		...
		mBulletBody.setLinearVelocity(_playerDir * 4, 0);
		...
		mShootSound.play();
		spawnExplosion2(player_self_sprite, playerDir, "bullet_self");
		
	}
	/**********************************************************************************
	 * 									<-
	 * Spawn explosion2 in front of given player and set UserData string (name) and
	 * play shoot sound
	 * 
	 **********************************************************************************/
	public void spawnExplosion2(AnimatedSprite _playerSprite, int _playerDir,
			String bulletName) {
		bulletCount++;
		...
		mEntityList.add(mBulletSprite);
		mShootSound.play();
		spawnExplosion3(player_self_sprite, playerDir, "bullet_self");

	}
	/**********************************************************************************
	 * 								/\
	 * Spawn explosion3 in front of given player and set UserData string (name) and
	 * play shoot sound
	 * 
	 **********************************************************************************/
	public void spawnExplosion3(AnimatedSprite _playerSprite, int _playerDir,
			String bulletName) {
		bulletCount++;
		...
		this.mPhysicsWorld.registerPhysicsConnector(new PhysicsConnector(
				mBulletSprite, mBulletBody, true, true));
		mBulletSprite.setUserData(bulletName + bulletCount);
		mBulletBody.setLinearVelocity(_playerDir * 0, 4);
		...
		mEntityList.add(mBulletSprite);
		mShootSound.play();
		spawnExplosion4(player_self_sprite, playerDir, "bullet_self");
	}
	/**********************************************************************************
	 * 							\/
	 * Spawn explosion2 in front of given player and set UserData string (name) and
	 ...
	 * 
	 **********************************************************************************/
	public void spawnExplosion4(AnimatedSprite _playerSprite, int _playerDir,
			String bulletName) {
		bulletCount++;
	...
	/**********************************************************************************
	  ...
	  * 
	  **********************************************************************************/
	 private boolean first_player_spawned = false;
	 public void spawnPlayer(String playerName, TiledTextureRegion playerTexture) {
	  AnimatedSprite mPlayerSprite;
	  playerCount++;
	  if(first_player_spawned){
	    mPlayerSprite = new AnimatedSprite(735, 200,
	     playerTexture);
	  }else{
	    mPlayerSprite = new AnimatedSprite(35, 200,
	     playerTexture);
	    first_player_spawned = true;
	  }
	...
	public void onPinchZoom(final PinchZoomDetector pPinchZoomDetector,
		...
		if (!isButtonAreaTouched && pZoomFactor > 0.4f
				&& mPinchZoomStartedCameraZoomFactor > 0.4f) {
	...
	public void onScroll(final ScrollDetector pScollDetector,
		...
		if (!isButtonAreaTouched) {
			this.mCamera.offsetCenter(-pDistanceX / zoomFactor * 10,
					-pDistanceY / zoomFactor * 10);
		}
	}
	...
	public void movePlayerRight(AnimatedSprite _playerSprite, Body _playerBody) {
		...
		_playerSprite.animate(new long[] { 50, 50, 50, 50 }, 4, 7,
				false);
	...
	public void movePlayerLeft(AnimatedSprite _playerSprite, Body _playerBody) {
		...
		_playerSprite.animate(new long[] { 50, 50, 50, 50 }, 8, 11,
				false);
	...
	public void movePlayerUp(AnimatedSprite _playerSprite, Body _playerBody) {
		...
		// if(playerName.equalsIgnoreCase("player_self")){
		_playerSprite.getTextureRegion().setFlippedHorizontal(true);
		_playerBody.setLinearVelocity( _playerBody.getLinearVelocity().x,-mLinearVelocityY);

		_playerSprite.animate(new long[] { 50, 50, 50, 50 }, 12, 15,
				false);
	...
	public void movePlayerDown(AnimatedSprite _playerSprite, Body _playerBody) {
		...
		_playerSprite.animate(new long[] { 50, 50, 50, 50 }, 0, 3,
				false);
	...
	public void stopPlayerX(AnimatedSprite _playerSprite, Body _playerBody) {
		// if (isPlayerMoving) {
		_playerBody.setLinearVelocity(0f, _playerBody.getLinearVelocity().y);
		// isPlayerMoving = false;}
	}

	public void stopPlayerY(AnimatedSprite _playerSprite, Body _playerBody) {
		// if (isPlayerMoving) {
		_playerBody.setLinearVelocity( _playerBody.getLinearVelocity().x,0f);
		// isPlayerMoving = false;}
	}
